...
pub struct Md5AudioHasher;

impl AudioHasher for Md5AudioHasher {
    fn audio_hash(reader: SourceReader) -> Result<String, Error> {
...
mod tests {
    ...
    fn test_audio_hash() {
        ...
        assert_eq!(
            Md5AudioHasher::audio_hash(SourceReader::FileReader(
                File::open(format!(
    ...
    fn test_audio_hash_invalid() {
        assert!(Md5AudioHasher::audio_hash(SourceReader::VecReader(vec![], 0)).is_err());
    }
}
